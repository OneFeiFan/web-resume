...
export default function ProjectDetail() {
  ...
  if (!project) return (
    <div style={{textAlign:'center',padding:'4rem 0'}}>
      <p style={{color:'#aaa'}}>项目未找到</p>
      <a onClick={() => navigate('/')} className="chip" style={{marginTop:'.5rem'}}>← 返回首页</a>
    </div>
  );

  return (
    <div>
      <a onClick={() => navigate('/')} style={{fontSize:'.78rem',display:'inline-block',marginBottom:'2rem'}}>← 返回首页</a>

      <h1 style={{color:'var(--c-primary)'}}>{project.name}</h1>
      <p className="proj-period" style={{marginTop:'.3rem'}}>{project.period} · {project.role}</p>
      <p className="card-summary" style={{marginTop:'.8rem',marginBottom:'1.5rem'}}>{project.summary}</p>
      ...
      {project.cases.map((c, i) => (
        <div className="adr" key={i} style={{borderColor:'var(--c-border)'}}>
          <h3><span className="adr-num">0{i + 1}</span> {c.title}</h3>
          <p style={{fontSize:'.82rem',color:'#666',marginTop:'.5rem'}}>
            <span className="case-label">背景</span> {c.background}
          </p>
          <p style={{fontSize:'.82rem',color:'#666'}}>
            <span className="case-label">决策</span> {c.decision}
          </p>
          <p style={{fontSize:'.82rem',color:'#666'}}>
            <span className="case-label">成果</span> {c.impact}
          </p>
          {c.commits.length > 0 && (
